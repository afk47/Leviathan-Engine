...
/*
	Credit TheCherno on github/youtube for this implementation
	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					LE_CORE_ERROR("Profiler::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			else
			{
				if (Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
					LE_CORE_ERROR("Profiler could not open results file '{0}'.", filepath);
				}
			}
		}
...
#define LE_PROFILE_BEGIN_SESSION(name, filepath) ::Leviathan::Profiler::Get().BeginSession(name, filepath)
#define LE_PROFILE_END_SESSION() ::Leviathan::Profiler::Get().EndSession()
#define LE_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::Leviathan::ProfilerUtils::CleanupOutputString(name, "__cdecl ");\
											   ::Leviathan::ProfilerTimer timer##line(fixedName##line.Data)
